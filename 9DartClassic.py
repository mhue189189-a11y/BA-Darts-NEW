...
def compare_nine_darter_log():
    sigmas = np.linspace(1, 60, 60)
    E_naive = []
    E_opt = []
    rT_naive = 103.0
    rD_naive = 166.0
    ...
    plt.semilogy(sigmas, E_naive, 's--', lw=2, label='Naiv (rT=103 mm, rD=166 mm)')
    plt.semilogy(sigmas, E_opt, 'o-', lw=2.5, label='Optimal/Greedy (Max pT + Max pD)')
    plt.xlabel("σ (mm)")
    plt.ylabel("9-Darter Wahrscheinlichkeit (log-Skala)")
    plt.title("9-Darter Erfolgswahrscheinlichkeit – Logarithmisch")
